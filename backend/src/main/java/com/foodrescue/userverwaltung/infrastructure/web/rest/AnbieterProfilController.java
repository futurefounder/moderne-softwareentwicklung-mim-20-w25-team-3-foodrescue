...
public class AnbieterProfilController {
    ...
    public AnbieterProfilController(AnbieterProfilApplicationService anbieterProfilApplicationService) {
    ...
    private Adresse baueAdresse(ErstelleAnbieterProfilRequest request) {
        ...
        Pattern p = Pattern.compile("^\\s*(.+?)\\s*,\\s*(\\d{4,6})\\s+(.+?)\\s*,\\s*([A-Za-z]{2,})\\s*$");
