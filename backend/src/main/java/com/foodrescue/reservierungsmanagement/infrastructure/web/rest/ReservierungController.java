...
public class ReservierungController {
  ...
  public ResponseEntity<CreateReservierungResponse> reserviere(
      @RequestBody ReserviereAngebotRequest request) {
    if (request == null || request.getAngebotId() == null || request.getAbholerId() == null) {
      return ResponseEntity.badRequest().build();
    }

    var cmd =
        new ReserviereAngebotCommand(
            new AngebotsId(request.getAngebotId()), new UserId(request.getAbholerId()));

    ReservierungsId id = service.reserviereAngebot(cmd);
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new CreateReservierungResponse(id.value()));
  }

  @GetMapping("/user/{userId}")
  public ResponseEntity<List<GeplanteAbholungResponse>> pickups(
