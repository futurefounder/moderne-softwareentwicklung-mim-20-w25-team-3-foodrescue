...
public class ReservierungsApplicationService {
    ...
    public ReservierungsId reserviereAngebot(ReserviereAngebotCommand command) {
        ...
        Angebot angebot = optionalAngebot.orElseThrow(() -> new IllegalArgumentException("Angebot nicht gefunden"));

        String abholerId = command.getAbholerId().getValue().toString();

        ReservierungsService domainService = new ReservierungsService(
    ...
    public List<GeplanteAbholungResponse>
    findeGeplanteAbholungenFuerUser(String userId) {
        ...
        return reservierungRepository.findeFuerAbholer(userId).stream()
                .map(r -> {
                    ...
                    if (angebot != null && angebot.getZeitfenster() != null) {
                        von = angebot.getZeitfenster().von() != null ? angebot.getZeitfenster().von().format(formatter) : null;
                        bis = angebot.getZeitfenster().bis() != null ? angebot.getZeitfenster().bis().format(formatter) : null;
                    }

                    return new com.foodrescue.reservierungsmanagement.infrastructure.web.rest.ReservierungController.GeplanteAbholungResponse(
                            r.getId(),
                            r.getAngebotId(),
                            titel,
                            beschreibung,
