...
public class AngebotApplicationService {
    ...
    public AngebotApplicationService(AngebotRepository repository, AngebotDomainService domainService) {
    ...
    public AngebotsId erstelleAngebot(ErstelleAngebotCommand cmd) {
        ...
        UserId anbieterId = new UserId(UUID.fromString(cmd.getAnbieterId()));  // <- passt zu Ihrer UserId :contentReference[oaicite:5]{index=5}
        ...
        AngebotsId id = AngebotsId.of(UUID.randomUUID().toString()); // oder Ihre bestehende Id-Erzeugung
        Angebot angebot = domainService.erstelleAngebot(
                id,
                anbieterId,
                cmd.getTitel(),
                cmd.getBeschreibung(),
